...
export const TopbarMenu = () => {
    ...
    return (
        ...
        <div className="flex flex-col bg-zinc-700/30 shadow-zinc-600 items-center justify-center">
            <div className={`w-3/4 m-auto flex lg:flex-row justify-between items-center ${session?.user.role == "admin" ? "border-b border-zinc-600" : ""}`}>
            <Link href="/home" className="p-2 flex items-center justify-start">
            <Image src={"/images/logo.png"}  alt="whiskers-logo" width={200} height={200} className="w-16 h-16"/>
            <h1 className="text-2xl lg:text-3xl ml-2">Whiskers</h1>
            </Link>
           ...
           {
                        session?.user.role == "admin" ? <div className="w-3/4 my-4 flex flex-row items-center justify-end gap-2">
                            <Link href={"/admin"} className="p-2 bg-zinc-200 text-zinc-800 rounded-2xl">Panel de control</Link>
